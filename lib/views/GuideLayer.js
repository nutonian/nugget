...
class GuideLayer extends Events {
    ...
    drawXLineGuide(dataSelection, enterSelection, chartOpts, guideOpts) {
        ...
        var line = opts.dataSelection
                        ...
                        // For the line to update, data must be returned as an array
    ...
    getPathY(length, pathNode, accuracyPx) {
        ...
        for (var i = 0; i < pathLength; i = i + accuracyPx) {
            var pos = pathNode.getPointAtLength(i);
            if (pos.x >= length) {
                y = pos.y;
                break;
            }
