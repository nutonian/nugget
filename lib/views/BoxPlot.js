...
class BoxPlot extends Graph {
    ...
    draw() {
        ...
        this.el = this.d3Svg.selectAll('.box')
                    ...
                    .each(function(d, i) {
                        ...
                        var halfRangeBand = (self.xRange(d.x) + (self.xRange.rangeBand() / 2));
                        var lineX2 = self.xRange(d.x) + self.xRange.rangeBand();
                        var initialX = self.xRange(d.x);
    ...
    update() {
        this.el.select('.range')
            .attr('x1', d => (this.xRange(d.x) + (this.xRange.rangeBand() / 2)))
            .attr('x2', d => (this.xRange(d.x) + (this.xRange.rangeBand() / 2)))
            .attr('y1', d => this.yRange(d.y_max))
            .attr('y2', d => this.yRange(d.y_min));

        this.el.select('.max')
            .attr('x1', d => this.xRange(d.x))
            .attr('x2', d => this.xRange(d.x) + this.xRange.rangeBand())
            .attr('y1', d => this.yRange(d.y_max))
