...
class Chart extends Events {
    constructor(options) {
        ...
        this.xGuideLabelFormat = options.xGuideLabelFormat;
        this.yGuideLabelFormat = options.yGuideLabelFormat;
        ...
        // Zoom hooks
        this.onZoomStart = options.onZoomStart || function() {};
        this.onZoomEnd   = options.onZoomEnd   || function() {};
        this.onZoomReset = options.onZoomReset || function() {};
    }
    ...
    */
    ...
    _updateZoom(isInitUpdate) {
        ...
        function updateAxisZoom(axis, range) {
            ...
            if (isInitUpdate) {
                ...
                if (axis === 'x') {
                    self[zoomKey].on('zoomstart.zoomHook', self._onZoomStart.bind(self));
                    self[zoomKey].on('zoomend.zoomHook', self._onZoomEnd.bind(self));
                }
            } else {
    ...
    _onZoomStart() {
        this.onZoomStart(d3.event.target);
    }

    _onZoomEnd() {
        this.onZoomEnd(d3.event.target);
    }
        ...
        this.d3Svg.on('mousedown.zoom', function() {
            ...
            if (clickTime - lastClickTime < DOUBLE_CLICK_MS) {
                ...
                if (self._aggregateDataRange.getXAxisType() !== Axes.AXIS_TYPES.ORDINAL) {
                    ...
                    self.zoomX.event(opts.zoomFixtureX);
                }
                ...
                self.zoomY.event(opts.zoomFixtureY);
