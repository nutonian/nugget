    ...
    _initBoxZoom(opts) {
        ...
        d3Svg.on('mousedown', function() {
            ...
            d3Svg.on('mouseup', function() {
                ...
                // Bail out if the user didn't move the mouse
                if (!endX || !endY) {
                    return;
                }

                var xRange = opts.xRange;
                var yRange = opts.yRange;

                if (!isOrdinal) {
                    var newXDomain = [xRange.invert(startX), xRange.invert(endX)].sort(d3.ascending);
                    xRange.domain(newXDomain);
                }

                var newYDomain = [yRange.invert(startY), yRange.invert(endY)].sort(d3.ascending);
                yRange.domain(newYDomain);
