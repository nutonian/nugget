...
class Chart extends Events {
    constructor(options) {
        ...
        this.padding       = (typeof options.padding === 'number') ? options.padding : 10;
    ...
    addGuideLayer(opts) {
        ...
        this._guideLayer = new GuideLayer();
        Object.assign(this._guideLayer, opts);
    ...
    update(isInitUpdate) {
        ...
        this._childElementMap.forEach(function(nuggetView) {
            nuggetView.drawElement(this._drawingSurface, this._xRange, this._yRange, this.axisLabels);
            nuggetView.initGuides(this._d3Svg, chartOpts);
        }, this);

        if (this._guideLayer) {
            if (!this._guideLayerEl) {
                this._guideLayerEl = this._d3Svg.append('g').attr('class', 'guide_layer');
            }
            this._guideLayer.init(this._guideLayerEl, chartOpts);
        }
    ...
    _updateLegend() {
        this.legendView.draw(this._legendEl, this.createLegendData());
    }
    ...
    _initLabelFormats() {
        ...
        if (this.xLabelFormat) {
            ...
            this.xLabelFormat = this._xRange.tickFormat ? this._xRange.tickFormat() : identityFn;
        }
        ...
        if (this.yLabelFormat) {
            ...
            this.yLabelFormat = this._yRange.tickFormat ? this._yRange.tickFormat() : identityFn;
        }
    }
    ...
    */
    ...
    _padAxes(xRange, yRange) {
        var paddingTop    = this.padding;
        var paddingBottom = this.padding;
        var paddingLeft   = this.padding;
        var paddingRight  = this.padding;

        // extra padding for legend, if this chart has one
        if (this.legend) {
            paddingTop = this.legendHeight + this.padding;
        }
    ...
    _drawXAxis() {
        this._d3Svg.select('.x_axis').remove();
    ...
    _drawYAxis() {
        this._d3Svg.select('.y_axis').remove();
    ...
    _createDrawingSurface(clipPathID) {
        var drawingSurface = this._d3Svg.append('g')
    ...
    _initLegend() {
        ...
        this._legendEl = this._d3Svg.append('svg')
                            .attr('class', 'legend')
    ...
    _setAxisLabels() {
        ...
        this._d3Svg.append('text')
            .attr('x', (this.width + this.margins.left) / 2)
            .attr('y', (this.height - (this.margins.bottom / 4)))
    ...
    */
    ...
    _initZoom() {
        ...
        var dataSeriesType = opts.dataSeriesType = this._aggregateDataRange.getXAxisType();
