...
class Chart extends Events {
    ...
    appendTo(selector) {
        var defaultDimensions = this._getDefaultDimensions(selector);
    ...
    _determineFullDataRange() {
        ...
        if (this.legend) {
            //pad the yMax to allow room for key
            var yRange = this._dataRange.yMax - this._dataRange.yMin;
            var keyFraction =  50 / (this.height - (this.margins.top + this.margins.bottom));
            var dataYPadding = yRange * keyFraction;

            this._dataRange.yMax += dataYPadding;
        }
    ...
    //drawing functions
    _getDefaultDimensions(selector) {
