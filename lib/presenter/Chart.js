...
class Chart extends Events {
    ...
    update(isInitUpdate) {
        ...
        this._childElementMap.forEach(function(nuggetView) {
            ...
            nuggetView.initGuides(this.d3Svg, chartOpts);
        }, this);
        ...
        if (this.guideLayer) {
            ...
            this.guideLayer.init(this._guideLayerEl, chartOpts);
        }
    ...
    _updateRanges() {
        ...
        if (this.xRange) {
            this.xRange.range(scales.x.range());
            this.xRange.domain(scales.x.domain());
        } else {
            this.xRange = scales.x;
        }

        if (this.yRange) {
            this.yRange.range(scales.y.range());
            this.yRange.domain(scales.y.domain());
        } else {
            this.yRange = scales.y;
        }
    }
    ...
    */
    ...
    _zoom(opts) {
        ...
        opts.children.forEach(function(view, id, map) {
            view.drawElement(opts.drawingSurface, opts.xRange, opts.yRange, this.axisLabels);
        }, this);
    }
